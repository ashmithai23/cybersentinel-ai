...
async def list_security_events(
    ...
    """
    Returns filterable, paginated security log event stream.
    """
    # Demo generated security events
    demo_events = [
    ...
    if search:
        s = search.lower()
        filtered = [e for e in filtered if s in e["source_ip"].lower() or s in e["attack_label"].lower() or s in e["uuid"].lower()]
    if protocol:
